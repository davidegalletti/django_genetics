...
def transcripts(request):
    ...
    try:
        mutalyzer_cs_endpoint = "https://v2.mutalyzer.nl/json/getTranscriptsByGeneName"
        url = mutalyzer_cs_endpoint + ("?build=hg38&name=%s" % request.GET['name'])
        aics = ApiInvoker(url=url)
        # https://v2.mutalyzer.nl/json/getTranscriptsByGeneName?build=hg19&name=MAP7D2
        """
            [
                "NM_001168465.1",
                "NM_001168466.1",
...
def sequence_variant(request):
    ...
    try:
        mutalyzer_cs_endpoint = "https://v2.mutalyzer.nl/json/checkSyntax"
        url = mutalyzer_cs_endpoint + ("?variant=%s" % request.GET['sv'])
        aics = ApiInvoker(url=url)
        # https://v2.mutalyzer.nl/json/checkSyntax?variant=AB026906.1:c.274de
        """
            {
              "valid": false,
              "messages": [
        ...
        if aics.decoded["valid"]:
            ...
            mutalyzer_ml_endpoint = "https://v2.mutalyzer.nl/json/runMutalyzerLight"
            ...
            if aiml.decoded['errors'] == 0:
                ...
                mutalyzer_gn_endpoint = "https://v2.mutalyzer.nl/json/getGeneName" #NM_001168465.1
                '''
                    https://v2.mutalyzer.nl/json/getGeneName?build=hg19&accno=NM_000052.6  ==> {"faultcode": "ENOTFOUND", "faultstring": "Transcript NM_000052.6 not found for build hg19."}
                    https://v2.mutalyzer.nl/json/getGeneName?build=hg19&accno=NM_000052.5  ==> "ATP7A"
                    https://v2.mutalyzer.nl/json/getGeneName?build=hg38&accno=NM_000052.6  ==> "ATP7A"
